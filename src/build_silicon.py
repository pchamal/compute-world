...
for c in chips:
    ...
    href = src_url(d["source_id"])
    src_lab = html.escape(src_name(d["source_id"]))
    src_a = f'<a href="{href}" rel="noopener">{src_lab}</a>' if href else src_lab
    price_title = f'{d["label"]} · {nice_date(d["as_of"])} · {src_name(d["source_id"])}'
    ...
    rows.append(f'''<tr class="chiprow" id="{html.escape(c["id"])}" data-id="{html.escape(c["id"])}" data-vendor="{html.escape(c["vendor"])}" data-rank="{c["rank"]}" data-score="{c["score"]}" data-price="{d["usd_per_gpu_hr"] if d["usd_per_gpu_hr"] is not None else ""}" tabindex="0" role="button" aria-expanded="false" aria-controls="drawer">
  ...
  <td class="price" data-col="price" title="{html.escape(price_title)}">
    <span class="px">{money(d["usd_per_gpu_hr"])}</span>
...
<title>The Silicon Tape · AI accelerator rental prices · compute.world</title>
<meta name="description" content="The Silicon Tape: a public index of AI accelerator spot and rental prices. Snapshot {S["updated"]}. SemiAnalysis prints as-of April 2026. No market cap. {html.escape(S["thesis"])}">
<link rel="canonical" href="https://compute.world/silicon.html">
<meta name="robots" content="index,follow,max-image-preview:large">
<meta property="og:title" content="The Silicon Tape · compute.world">
<meta property="og:description" content="{html.escape(S["thesis"])}">
<meta property="og:url" content="https://compute.world/silicon.html">
...
<div class="wrap">
  <div class="masthead">
    ...
    <div class="sub">The Silicon Tape · public rental tape · snapshot {S["updated"]}</div>
    <div class="mastrule"></div>
  </div>

  <div class="lede">
    <h1>The rental tape, <em>split</em>.</h1>
    <p class="standfirst">{html.escape(S["thesis"])} Announcements are cheap. Megawatts and GPU-hours are not. This is not a market cap. Scores are ordinal hygiene.</p>
    <div class="subrow">
...
{chr(10).join(rows)}
  ...
  <details class="meth" open>
    ...
    <div class="mb">
      ...
      <ul>{omitted}</ul>
      <p>Machine-readable: <a href="/silicon.json">silicon.json</a> (CC BY 4.0, attribution to compute.world). Cite as: <code>{html.escape(S["cite"])}</code>. Corrections: <a href="/contact.html">get in touch</a>. Sibling of the <a href="/">Compute Net Worth Index</a>.</p>
  ...
  <div class="colophon">
    ...
    <div class="c2">The Silicon Tape · v0 · snapshot {S["updated"]} · part of The Compute Net Worth Index&#8482; · &copy; 2026 Pukar C. Hamal · San Francisco, CA</div>
  </div>
</div>
...
rss_items = "".join(f'''
  <item>
    <title>{html.escape(c["name"])} · {money(c["display"]["usd_per_gpu_hr"])} {html.escape(c["display"]["label"])}</title>
